    ...
    _tsneParametersTopLevel(nullptr)
{
    connect(&_tsneAnalysis, &TsneAnalysis::finished, this, [this]() {
        //qDebug() << "<<<<<<<<<<HsneScaleAction: tsne analysis finished";
        if (!_refineEmbeddings.empty())
        {
            if (_dualAnalysisPlugin) {
                _dualAnalysisPlugin->onRefineFinished(_refineEmbeddings.back());
                //qDebug() << "Sent refined embedding to DualAnalysisPlugin.";
            }
            else
                qDebug() << "Plugin ptr is null";
        }
        else
            qWarning() << "No refined embeddings available to send to DualAnalysisPlugin.";
        });
}

HsneScaleAction::HsneScaleAction(QObject* parent, DualAnalysisPlugin* plugin, HsneHierarchy& hsneHierarchy, Dataset<Points> inputDataset, Dataset<Points> embeddingDataset, TsneParameters* tsneParametersTopLevel) :
    HsneScaleAction(parent, plugin, hsneHierarchy, inputDataset, embeddingDataset)
{
    _tsneParametersTopLevel = tsneParametersTopLevel;
    initLayoutAndConnection();
}

HsneScaleAction::HsneScaleAction(QObject* parent, DualAnalysisPlugin* plugin, HsneHierarchy& hsneHierarchy, Dataset<Points> inputDataset, Dataset<Points> embeddingDataset, unsigned int scale) :
    HsneScaleAction(parent, plugin, hsneHierarchy, inputDataset, embeddingDataset)
{
...
void HsneScaleAction::refine()
{
    ...
    // Create a new data set for the embedding
    {
        ...
        _refinedScaledActions.push_back(new HsneScaleAction(this, _dualAnalysisPlugin, _hsneHierarchy, _input, refineEmbedding, refinedScaleLevel));
    ...
    //qDebug() << "<<<<<<<<<<HsneScaleAction tsne analysis started";
...
void HsneScaleAction::fromVariantMap(const QVariantMap& variantMap)
{
    ...
    for (const auto& refinedEmbeddingMapVar : variantMap["refinedEmbeddingsMap"].toMap())
    {
        ...
        _refinedScaledActions.push_back(new HsneScaleAction(this, _dualAnalysisPlugin, _hsneHierarchy, _input, refineEmbedding, refinedEmbeddingMap["refinedCurrentScaleLevel"].toUInt()));
