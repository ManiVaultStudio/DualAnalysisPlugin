...
GeneralTsneSettingsAction::GeneralTsneSettingsAction(TsneSettingsAction& tsneSettingsAction) :
    ...
    _numDimensionOutputAction(this, "Output dimension")
{
    addAction(&_knnAlgorithmAction);
    addAction(&_distanceMetricAction);
    addAction(&_perplexityAction);

    addAction(&_numDimensionOutputAction);
    _numDimensionOutputAction.initialize(QStringList({ "1", "2" }), "2");
    _numDimensionOutputAction.setToolTip("Output dimension of the t-SNE algorithm.");
    ...
    const auto updateNumDimensionOutput = [this]() -> void {
        if (_numDimensionOutputAction.getCurrentText() == "1")
        {
            _tsneSettingsAction.getTsneParameters().setNumDimensionsOutput(1);
            _tsneSettingsAction.getTsneParameters().setGradientDescentType(GradientDescentType::CPU);
        }
        if (_numDimensionOutputAction.getCurrentText() == "2")
        {
            _tsneSettingsAction.getTsneParameters().setNumDimensionsOutput(2);
            _tsneSettingsAction.getTsneParameters().setGradientDescentType(GradientDescentType::GPU);
        }        
    };
    ...
    const auto updateReadOnly = [this]() -> void {
        ...
        _numDimensionOutputAction.setEnabled(enable);
    };
    ...
    connect(&_numDimensionOutputAction, &OptionAction::currentIndexChanged, this, [this, updateNumDimensionOutput](const std::int32_t& currentIndex) {
        updateNumDimensionOutput();
        });
