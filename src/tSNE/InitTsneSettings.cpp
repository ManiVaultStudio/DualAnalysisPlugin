...
std::vector<float> InitTsneSettings::getInitEmbedding(size_t numPoints)
{
    ...
    int numDimensionsOutput = _tsneSettingsAction.getGeneralTsneSettingsAction().getNumDimensionOutputAction().getCurrentText().toInt();

    //std::vector<float> initPositions(numPoints * 2, -1.f);
    std::vector<float> initPositions(numPoints * numDimensionsOutput, -1.f);
    ...
    if (_randomInitAction.isChecked())
    {
        ...
        if (numDimensionsOutput == 1)
        {
            // 1D embedding
            for (size_t i = 0; i < numPoints; ++i) {
                initPositions[i] = dis(gen);
            }
        }
    }
    else
    {
        ...
        if (numDimensionsOutput == 1)
            initData->populateDataForDimensions(initPositions, std::vector<int32_t>{xDim}); // 1D embedding
    ...
    if (_rescaleInitAction.isChecked())
    {
        ...
        {
            // 1D embedding
            float sum = std::accumulate(initPositions.begin(), initPositions.end(), 0.0f);
            float mean = sum / numPoints;
            float stdevCurrent = 0.0f;
            for (size_t i = 0; i < numPoints; ++i)
                stdevCurrent += std::pow(initPositions[i] - mean, 2);
            stdevCurrent = std::sqrt(stdevCurrent / numPoints);
            float scaleFactor = stdevDesired / stdevCurrent;
            for (float& position : initPositions)
                position = (position - mean) * scaleFactor + mean;
        }
    }
