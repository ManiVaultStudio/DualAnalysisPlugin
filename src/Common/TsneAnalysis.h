...
public:
    ...
    // Special for computing 1D embedding with alignment with 2D embedding as master
    TsneWorker(TsneParameters tsneParameters, KnnParameters knnParameters, const std::vector<float>& data, uint32_t numDimensions, const hdi::data::Embedding<float>::scalar_vector_type* initEmbedding, const hdi::data::Embedding<float>::scalar_vector_type* masterEmbedding);
    
    // Special for computing 1D embedding with alignment with 1D embedding as master plus connections
    TsneWorker(TsneParameters tsneParameters, KnnParameters knnParameters, const std::vector<float>& data, uint32_t numDimensions, const hdi::data::Embedding<float>::scalar_vector_type* initEmbedding, const hdi::data::Embedding<float>::scalar_vector_type* masterEmbedding, const std::vector<std::unordered_map<uint32_t, uint32_t>> connections);
...
private:
    ...
    // test 1D embedding alignment with master embedding
    hdi::dr::EmbeddingEqualizer<float>     _eqTest;
    hdi::data::Embedding<float>		       _masterEmbedding;

    // test 1D embedding alignment with 1D embedding as master and with connections
    std::vector<std::unordered_map<uint32_t, uint32_t>> _connections;

    // test flag
    bool _test = false; // TEMP hard coded for no singleTSNEIteration when aligning 2DA to 1DA when iteration < 500
...
public: // Interactions
    ...
    // Special for computing 1D embedding with alignment with 2D embedding as master
    void startComputation(TsneParameters parameters, KnnParameters knnParameters, std::vector<float>&& data, uint32_t numDimensions, const hdi::data::Embedding<float>::scalar_vector_type* initEmbedding, const hdi::data::Embedding<float>::scalar_vector_type* masterEmbedding);
    // Special for computing 1D embedding with alignment with 1D embedding as master plus connections
    void startComputation(TsneParameters parameters, KnnParameters knnParameters, std::vector<float>&& data, uint32_t numDimensions, const hdi::data::Embedding<float>::scalar_vector_type* initEmbedding, const hdi::data::Embedding<float>::scalar_vector_type* masterEmbedding, const std::vector<std::unordered_map<uint32_t, uint32_t>> connections);
