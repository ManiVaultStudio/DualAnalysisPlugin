...
TsneWorker::TsneWorker(TsneParameters tsneParameters, KnnParameters knnParameters, const std::vector<float>& data, uint32_t numDimensions, const hdi::data::Embedding<float>::scalar_vector_type* initEmbedding,
    const hdi::data::Embedding<float>::scalar_vector_type* masterEmbedding) :
    TsneWorker(tsneParameters)
{
    // Special for computing an embedding with alignment with an embedding as master - assume  master 2D + slave 1D, master 1D + slave 2D
    _knnParameters = knnParameters;
    assert(numDimensions > 0);
    _numPoints = data.size() / numDimensions;
    _numDimensions = numDimensions;
    ...
    if (_tsneParameters.getNumDimensionsOutput() == 1)//master 2D + slave 1D
    { 
    _masterEmbedding = { 2,  static_cast<uint32_t>(masterEmbedding->size() / 2) }; 
    _masterEmbedding.getContainer() = *masterEmbedding;
    qDebug() << "master 2D + slave 1D";
    }
    else if (_tsneParameters.getNumDimensionsOutput() == 2) //master 1D + slave 2D
    {
        _masterEmbedding = { 1,  static_cast<uint32_t>(masterEmbedding->size()) }; 
        _masterEmbedding.getContainer() = *masterEmbedding;
        qDebug() << "master 1D + slave 2D";
        _test = true;
    }
    ...
    if (initEmbedding)
        qDebug() << "<<<<<<<<<<<<init Embedding!";

    _eqTest.initialize(&_masterEmbedding, &_embedding);
    qDebug() << "<<<<<<<<<<<<set tsne worker with master embedding";
        
}

TsneWorker::TsneWorker(TsneParameters tsneParameters, KnnParameters knnParameters, const std::vector<float>& data, uint32_t numDimensions, const hdi::data::Embedding<float>::scalar_vector_type* initEmbedding, 
    const hdi::data::Embedding<float>::scalar_vector_type* masterEmbedding, const std::vector<std::unordered_map<uint32_t, uint32_t>> connections) :
    TsneWorker(tsneParameters)
{
    // Special for computing 1D embedding with alignment with 1D embedding as master plus connections
    _knnParameters = knnParameters;
    assert(numDimensions > 0);
    _numPoints = data.size() / numDimensions;
    _numDimensions = numDimensions;
    ...
    _masterEmbedding = { 1,  static_cast<uint32_t>(masterEmbedding->size()) }; // TODO: hard coded, assume master embedding is 1D
    _masterEmbedding.getContainer() = *masterEmbedding;

    if (_tsneParameters.getNumDimensionsOutput() == 1)
        qDebug() << "master 1D + slave 1D + connections";
    else if (_tsneParameters.getNumDimensionsOutput() == 2)
    {
        //_test = true;
        qDebug() << "master 1D + slave 2D + connections";
    }
    ...
    _eqTest.initialize(&_masterEmbedding, &_embedding, &_connections);
    qDebug() << "<<<<<<<<<<<<set tsne worker with master embedding plus connections";
...
void TsneWorker::computeGradientDescent(uint32_t iterations)
{
    ...
    double t_grad = 0;
    {
        ...
        for (_currentIteration = beginIteration; _currentIteration < endIteration; ++_currentIteration) {
            ...
            {
                //if (_currentIteration > 500)
                singleTSNEIteration();
            }
                

            // TODO: specially for alignment, add equalizer here
            if (_masterEmbedding.numDataPoints() > 0 && _currentIteration < 500)
            {
                int iteration = _currentIteration;
                double mult = 1 - double(iteration) / 500;
                _eqTest.doAnIteration(mult);
            }
...
void TsneAnalysis::startComputation(TsneParameters parameters, KnnParameters knnParameters, std::vector<float>&& data, uint32_t numDimensions, const hdi::data::Embedding<float>::scalar_vector_type* initEmbedding, const hdi::data::Embedding<float>::scalar_vector_type* masterEmbedding)
{
    // test for 1D tsne with 2D as master
    deleteWorker();

    _tsneWorker = new TsneWorker(parameters, knnParameters, std::move(data), numDimensions, initEmbedding, masterEmbedding);
...
void TsneAnalysis::startComputation(TsneParameters parameters, KnnParameters knnParameters, std::vector<float>&& data, uint32_t numDimensions, const hdi::data::Embedding<float>::scalar_vector_type* initEmbedding,
    const hdi::data::Embedding<float>::scalar_vector_type* masterEmbedding, const std::vector<std::unordered_map<uint32_t, uint32_t>> connections)
{
    // test for 1D tsne with alignment from 1D plus connections
    deleteWorker();

    _tsneWorker = new TsneWorker(parameters, knnParameters, std::move(data), numDimensions, initEmbedding, masterEmbedding, connections);
